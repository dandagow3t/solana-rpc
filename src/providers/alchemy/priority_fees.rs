...
use super::types::{AlchemyPriorityFeeOptions, AlchemyPriorityFeeParams, AlchemyPriorityFeeResult};
...
#[async_trait]
impl PriorityFeeProvider for AlchemyProvider {
    async fn get_priority_fee_estimate(
        &self,
        request: PriorityFeeRequest,
    ) -> Result<PriorityFeeEstimate, RpcError> {
        // Alchemy requires EITHER priorityLevel OR includeAllPriorityFeeLevels, not both
        ...
        let params = AlchemyPriorityFeeParams {
            account_keys: request.account_keys,
            options: Some(AlchemyPriorityFeeOptions {
                priority_level: priority_level_str.map(|s| s.to_string()),
                include_all_priority_fee_levels: if priority_level_str.is_none() {
                    Some(true)
                } else {
                    None
                },
        ...
        let rpc_response: JsonRpcResponse<AlchemyPriorityFeeResult> =
        ...
        // When includeAllPriorityFeeLevels is true, Alchemy returns only priorityFeeLevels
        // and not a single estimate. We use the medium level as the recommended estimate.
        // When a specific priority level is requested, Alchemy returns a single priorityFee value.
